...
PGRAPHIC g_Graphics = std::make_unique<CGraphicDev>();

CGraphicDev::CGraphicDev(void)
	: m_Device(nullptr)
	, m_Factory(nullptr), m_SwapChain(nullptr), m_Fence(nullptr)
	, m_CommandQueue(nullptr), m_CommandAllocator(nullptr), m_CommandList(nullptr)
	, m_DepthStencilBuffer(nullptr), m_RtvHeap(nullptr), m_DsvHeap(nullptr)
	, m_iCurrBackBuffer(0), m_iRtvDescriptiorSize(0), m_iDsvDescriptiorSize(0), m_CbvSrvUavDescriptorSize(0), m_iCurrentFence(0)
	, mMainWndCaption(L"D3D_App")
...
bool CGraphicDev::Initialize(void)
{
	if (!Init_Graphic())
		return FALSE;
