...
bool BoxScene::Update(const float & dt)
{
...
bool BoxScene::Render(const float & dt)
{
...
void BoxScene::BuildConstantBuffers(void)
{
	...
	int BoxSceneCBufIndex = 0;
	cbAddress += BoxSceneCBufIndex * objCBByteSize;
...
void BoxScene::BuildRootSignature(void)
{
...
void BoxScene::BuildShadersAndInputLayout(void)
{
	HRESULT hr = S_OK;
...
void BoxScene::BuildBoxGeometry(void)
{
...
std::shared_ptr<BoxScene> BoxScene::Create(void)
{
	auto pBoxScene = std::make_shared<BoxScene>();
	if (!pBoxScene)	return nullptr;
