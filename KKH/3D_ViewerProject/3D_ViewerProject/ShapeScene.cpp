...
bool ShapeScene::Update(const float & dt)
{
	OnKeyboardInput(dt);
	UpdateCamera(dt);
...
bool ShapeScene::Render(const float & dt)
{
...
void ShapeScene::BuildConstantBufferViews(void)
{
...
void ShapeScene::BuildRootSignature(void)
{	
...
void ShapeScene::BuildShadersAndInputLayout(void)
{
	HRESULT hr = S_OK;
...
void ShapeScene::BuildShapeGeometry(void)
{
...
void ShapeScene::BuildRenderItems(void)
{
	...
	boxRitem->Geo = mGeometries["ShapeSceneGeo"].get();
	...
	gridRitem->Geo = mGeometries["ShapeSceneGeo"].get();
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		leftCylRitem->Geo = mGeometries["ShapeSceneGeo"].get();
		...
		rightCylRitem->Geo = mGeometries["ShapeSceneGeo"].get();
		...
		leftSphereRitem->Geo = mGeometries["ShapeSceneGeo"].get();
		...
		rightSphereRitem->Geo = mGeometries["ShapeSceneGeo"].get();
...
void ShapeScene::DrawRenderItems(ID3D12GraphicsCommandList * cmdList, const std::vector<RenderItem*>& ritems)
{
	UINT objCBByteSize = D3DUTIL.CalcConstantBufferByteSize(sizeof(ObjectConstants));
...
void ShapeScene::OnKeyboardInput(const float & dt)
{
...
void ShapeScene::UpdateCamera(const float & dt)
{
...
void ShapeScene::UpdateObjectCBs(const float & dt)
{
...
void ShapeScene::UpdateMainPassCB(const float & dt)
{
...
std::shared_ptr<ShapeScene> ShapeScene::Create(void)
{
	SHAPE pShape = std::make_shared<ShapeScene>();
